...
function App(){
  ...
  const saveProduct=async(e)=>{
    ...
    const payload={sku:autoSku,name:f.get('name').trim(),category_id:f.get('category_id')||null,unit:f.get('unit').trim()||'ชิ้น',min_stock:Math.max(0,Number(f.get('min_stock')||0)),is_active:f.get('is_active')==='true'}
    try{
      let result
...
const forceDeleteProduct = async product => {
  ...
  const filteredProducts=useMemo(()=>products.filter(p=>{
    ...
    const matchesQuery=`${p.name} ${p.categories?.name||''}`.toLowerCase().includes(query.toLowerCase())
  ...
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><div className="brand-mark small">B</div><div><strong>{company?.company_name||'BlueWell Inventory'}</strong><small>{company?.subtitle||'ระบบจัดการสต็อกออนไลน์'}</small></div></div><nav>{nav.map(([id,label,Icon])=><button key={id} className={page===id?'active':''} onClick={()=>setPage(id)}><Icon size={19}/>{label}</button>)}<button type="button" className="mobile-logout" onClick={logout}><LogOut size={19}/>ออกจากระบบ</button></nav><div className="user-card"><CircleUserRound/><div className="user-info"><strong>{displayName}</strong><small>{isAdmin?'ผู้ดูแลระบบ':'ผู้จัดการคลัง'}</small></div><button className="icon-btn" onClick={logout} title="ออกจากระบบ" aria-label="ออกจากระบบ"><LogOut size={18}/></button></div></aside>
    ...
    {page==='products'&&<div className="content products-page"><section className="panel"><div className="panel-title products-panel-title"><div><h2>สินค้า</h2><p>จัดการข้อมูลสินค้าและยอดคงเหลือ</p></div><div className="toolbar products-toolbar"><select className="product-status-filter" value={productStatusFilter} onChange={e=>setProductStatusFilter(e.target.value)} aria-label="กรองสถานะสินค้า"><option value="all">ทุกสถานะ</option><option value="active">ใช้งานปกติ</option><option value="inactive">ไม่ใช้งานแล้ว</option></select><div className="search product-search"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="ค้นหาสินค้า"/></div>{isAdmin&&<button className="btn primary product-add-button" onClick={()=>{setEditing(null);setImagePreview('');setProductModal(true)}}><Plus size={18}/>เพิ่มสินค้า</button>}</div></div><div className="table-wrap"><table><thead><tr><th>สินค้า</th><th>หมวดหมู่</th><th>คงเหลือ</th><th>แจ้งเตือนเมื่อเหลือ</th><th>สถานะสต็อก</th><th>สถานะสินค้า</th><th>จัดการ</th></tr></thead><tbody>{filteredProducts.map(p=>{const isLow=p.is_active&&Number(p.min_stock||0)>0&&Number(p.quantity||0)<=Number(p.min_stock||0);return <tr key={p.id} className={`${isLow?'low-stock-row':''} ${!p.is_active?'inactive-product-row':''}`}><td><div className="product-name-cell">{p.image_path?<img className="product-thumb" src={productImageUrl(p.image_path)} alt={p.name}/>:<span className="product-thumb placeholder"><ImageIcon size={18}/></span>}<strong>{p.name}</strong></div></td><td>{p.categories?.name||'-'}</td><td><strong>{Number(p.quantity||0).toLocaleString()}</strong> {p.unit}</td><td>{Number(p.min_stock||0)>0?`${Number(p.min_stock).toLocaleString()} ${p.unit}`:'ไม่แจ้งเตือน'}</td><td>{p.is_active?<span className={`badge ${isLow?'danger':'success'}`}>{isLow?'ใกล้หมด':'ปกติ'}</span>:<span className="badge neutral">หยุดติดตาม</span>}</td><td><span className={`badge ${p.is_active?'success':'danger'}`}>{p.is_active?'ใช้งานปกติ':'ไม่ใช้งานแล้ว'}</span></td><td><div className="row-actions"><button className="btn mini" disabled={!p.is_active} title={p.is_active?'ทำรายการสต็อก':'สินค้านี้ไม่ใช้งานแล้ว'} onClick={()=>{setTxProduct(p);setTxModal(true)}}><ArrowUpFromLine size={15}/>เบิก/เติม</button>{isAdmin&&<button className="icon-btn" title="แก้ไขสินค้า" onClick={()=>{setEditing(p);setImagePreview(productImageUrl(p.image_path));setProductModal(true)}}><Pencil size={16}/></button>}{isAdmin&&developmentMode&&<button className="icon-btn danger-button" title="ลบข้อมูลทดลอง" onClick={()=>forceDeleteProduct(p)}><Trash2 size={16}/></button>}</div></td></tr>})}</tbody></table>{!filteredProducts.length&&<Empty text="ไม่พบสินค้าตามสถานะที่เลือก"/>}</div></section></div>}
