...
const PREPACK_LABELS = { pack:'เข้าพรีแพ็ค', ship:'พร้อมส่งแล้ว', return:'คืนคลังกลาง' }
const prepackBadge = type => type === 'pack' ? 'success' : type === 'return' ? 'warning' : 'neutral'
...
function App(){
  ...
  const loadAll = useCallback(async(user)=>{
    ...
    try{
      ...
      const [prepackInventoryRes,prepackTxRes]=await Promise.all([
        supabase.from('prepack_inventory').select('*, products(*, categories(name))').order('updated_at',{ascending:false}),
        supabase.from('prepack_transactions').select('*, products(name,unit)').order('created_at',{ascending:false}).limit(300),
      ])
      const prepackUnavailable=Boolean(prepackInventoryRes.error||prepackTxRes.error)
      setPrepackReady(!prepackUnavailable)
      setPrepackInventory(prepackUnavailable?[]:(prepackInventoryRes.data||[]))
  ...
  useEffect(()=>{
    ...
    const channel=supabase.channel('bluewell-live')
      ...
      .on('postgres_changes',{event:'*',schema:'public',table:'prepack_inventory'},()=>loadAll(session.user))
      .on('postgres_changes',{event:'*',schema:'public',table:'prepack_transactions'},()=>loadAll(session.user))
  ...
  const saveProduct=async(e)=>{
    ...
    const initialQuantity=Math.max(0,Number(f.get('case_count')||0))*unitsPerCase+Math.max(0,Number(f.get('loose_quantity')||0))
    const payload={sku:autoSku,name:f.get('name').trim(),category_id:editing?.category_id||null,unit:'ชิ้น',units_per_case:unitsPerCase,min_stock:Math.max(0,Number(f.get('min_stock')||0)),is_active:f.get('is_active')==='true'}
    try{
      let result
      if(editing) result=await supabase.from('products').update(payload).eq('id',editing.id).select().single()
      else result=await supabase.from('products').insert({...payload,quantity:initialQuantity,created_by:session.user.id}).select().single()
  ...
  const saveTx=async(e)=>{
    ...
    const f=new FormData(e.currentTarget)
    try{
      if(!txProduct?.is_active) throw new Error('สินค้านี้ถูกกำหนดเป็นไม่ใช้งานแล้ว จึงไม่สามารถทำรายการสต็อกได้')
      const inputQuantity=Number(f.get('quantity'))
      const isCase=f.get('quantity_mode')==='case'
      const quantity=inputQuantity*(isCase?Math.max(1,Number(txProduct.units_per_case||1)):1)
      const conversionNote=isCase?`[${inputQuantity.toLocaleString()} ลัง × ${Number(txProduct.units_per_case||1).toLocaleString()} ชิ้น] `:''
      const {error}=await supabase.rpc('process_stock_transaction',{p_product_id:txProduct.id,p_transaction_type:f.get('type'),p_quantity:quantity,p_note:`${conversionNote}${f.get('note')||''}`.trim()})
      if(error) throw error
  ...
  const savePrepack=async(e)=>{
    ...
    const f=new FormData(e.currentTarget)
    try{
      if(!prepackReady) throw new Error('กรุณารัน migration-v3.1-prepack.sql ใน Supabase ก่อน')
      const productId=prepackAction==='pack'?f.get('product_id'):prepackProduct?.id
      if(!productId) throw new Error('กรุณาเลือกสินค้า')
      const selectedProduct=prepackAction==='pack'?products.find(p=>p.id===productId):prepackProduct
      const inputQuantity=Number(f.get('quantity'))
      const isCase=f.get('quantity_mode')==='case'
      const quantity=inputQuantity*(isCase?Math.max(1,Number(selectedProduct?.units_per_case||1)):1)
      const conversionNote=isCase?`[${inputQuantity.toLocaleString()} ลัง × ${Number(selectedProduct?.units_per_case||1).toLocaleString()} ชิ้น] `:''
      ...
      if(error) throw error
      setPrepackModal(false);setPrepackProduct(null)
      notify(prepackAction==='pack'?'ย้ายสินค้าเข้าพรีแพ็คสำเร็จ':prepackAction==='ship'?'ตัดยอดพร้อมส่งสำเร็จ':'คืนสินค้าเข้าคลังกลางสำเร็จ')
      await loadAll(session.user)
    }catch(error){notify(`ทำรายการพรีแพ็คไม่สำเร็จ: ${error.message}`)}finally{setBusy(false)}
  }
...
const forceDeleteProduct = async product => {
  ...
  const filteredProducts=useMemo(()=>products.filter(p=>{
    const matchesStatus=productStatusFilter==='all'||(productStatusFilter==='active'?p.is_active:!p.is_active)
    const matchesQuery=String(p.name||'').toLowerCase().includes(query.toLowerCase())
    return matchesStatus&&matchesQuery
  }),[products,query,productStatusFilter])
  const filteredPrepack=useMemo(()=>prepackInventory
    .filter(row=>Number(row.quantity||0)>0)
    .filter(row=>`${row.products?.name||''} ${row.products?.categories?.name||''}`.toLowerCase().includes(prepackQuery.toLowerCase())),
  [prepackInventory,prepackQuery])
  const prepackSummary=useMemo(()=>{
    const total=prepackInventory.reduce((sum,row)=>sum+Number(row.quantity||0),0)
    const skuCount=prepackInventory.filter(row=>Number(row.quantity||0)>0).length
    const today=new Date();today.setHours(0,0,0,0)
    const shippedToday=prepackTransactions
      .filter(t=>t.transaction_type==='ship'&&new Date(t.created_at)>=today)
  ...
  const reportSummary=useMemo(()=>{
    ...
    const currentStock=products
      .filter(p=>(!reportProduct||p.id===reportProduct)&&(!reportCategory||p.category_id===reportCategory))
      .reduce((sum,p)=>sum+Number(p.quantity||0),0)
    return {restock,returned,issue,net:restock+returned-issue,currentStock,count:reportRows.length}
  ...
  const nav=[['dashboard','ภาพรวม',LayoutDashboard],['products','สินค้า',Warehouse],['prepack','พรีแพ็ค',PackageCheck],['history','ประวัติ',History],['claims','ส่งเคลม',ShieldAlert],['reports','รายงาน',BarChart3],...(isAdmin?[['settings','ตั้งค่า',Settings]]:[])]
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><div className="brand-mark small">B</div><div><strong>{company?.company_name||'BlueWell Inventory'}</strong><small>{company?.subtitle||'ระบบจัดการสต็อกออนไลน์'}</small></div></div><nav>{nav.map(([id,label,Icon])=><button key={id} className={page===id?'active':''} onClick={()=>setPage(id)}><Icon size={19}/>{label}</button>)}<button type="button" className="mobile-logout" onClick={logout}><LogOut size={19}/>ออกจากระบบ</button></nav><div className="user-card"><CircleUserRound/><div className="user-info"><strong>{displayName}</strong><small>{isAdmin?'ผู้ดูแลระบบ':'พนักงาน'}</small></div><button className="icon-btn" onClick={logout} title="ออกจากระบบ" aria-label="ออกจากระบบ"><LogOut size={18}/></button></div></aside>
    ...
    {page==='products'&&<div className="content products-page"><section className="panel">
      <div className="panel-title products-panel-title">
        <div><h2>สินค้า</h2><p>จัดการสินค้าแบบลัง และแปลงยอดคงเหลือเป็นจำนวนชิ้นอัตโนมัติ</p></div>
        <div className="toolbar products-toolbar">
          <select className="product-status-filter" value={productStatusFilter} onChange={e=>setProductStatusFilter(e.target.value)} aria-label="กรองสถานะสินค้า"><option value="all">ทุกสถานะ</option><option value="active">ใช้งานปกติ</option><option value="inactive">ไม่ใช้งานแล้ว</option></select>
          <div className="search product-search"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="ค้นหาสินค้า"/></div>
          {isAdmin&&<button className="btn primary product-add-button" onClick={()=>{setEditing(null);setImagePreview('');setCaseCount(0);setPiecesPerCase(1);setLoosePieces(0);setProductModal(true)}}><Plus size={18}/>เพิ่มสินค้า</button>}
        </div>
      </div>
      <div className="table-wrap"><table className="products-case-table">
        <thead><tr><th>สินค้า</th><th>จำนวนลัง</th><th>จำนวนต่อลัง</th><th>คงเหลือ</th><th>แจ้งเตือนเมื่อเหลือ</th><th>สถานะสต็อก</th><th>สถานะสินค้า</th><th>จัดการ</th></tr></thead>
        <tbody>{filteredProducts.map(p=>{const unitsPerCase=Math.max(1,Number(p.units_per_case||1));const fullCases=Math.floor(Number(p.quantity||0)/unitsPerCase);const remainder=Number(p.quantity||0)%unitsPerCase;const isLow=p.is_active&&Number(p.min_stock||0)>0&&Number(p.quantity||0)<=Number(p.min_stock||0);return <tr key={p.id} className={`${isLow?'low-stock-row':''} ${!p.is_active?'inactive-product-row':''}`}>
          <td><div className="product-name-cell">{p.image_path?<img className="product-thumb" src={productImageUrl(p.image_path)} alt={p.name}/>:<span className="product-thumb placeholder"><ImageIcon size={18}/></span>}<div><strong>{p.name}</strong>{remainder>0&&<small>เศษ {remainder.toLocaleString()} ชิ้น</small>}</div></div></td>
          <td><strong>{fullCases.toLocaleString()}</strong> ลัง</td>
          <td>{unitsPerCase.toLocaleString()} ชิ้น/ลัง</td>
          <td><strong>{Number(p.quantity||0).toLocaleString()}</strong> ชิ้น</td>
          <td>{Number(p.min_stock||0)>0?`${Number(p.min_stock).toLocaleString()} ชิ้น`:'ไม่แจ้งเตือน'}</td>
          <td>{p.is_active?<span className={`badge ${isLow?'danger':'success'}`}>{isLow?'ใกล้หมด':'ปกติ'}</span>:<span className="badge neutral">หยุดติดตาม</span>}</td>
          <td><span className={`badge ${p.is_active?'success':'danger'}`}>{p.is_active?'ใช้งานปกติ':'ไม่ใช้งานแล้ว'}</span></td>
          <td><div className="row-actions"><button className="btn mini" disabled={!p.is_active} title={p.is_active?'ทำรายการสต็อก':'สินค้านี้ไม่ใช้งานแล้ว'} onClick={()=>{setTxProduct(p);setTxModal(true)}}><ArrowUpFromLine size={15}/>เบิก/เติม</button>{isAdmin&&<button className="icon-btn" title="แก้ไขสินค้า" onClick={()=>{setEditing(p);setPiecesPerCase(Math.max(1,Number(p.units_per_case||1)));setImagePreview(productImageUrl(p.image_path));setProductModal(true)}}><Pencil size={16}/></button>}{isAdmin&&developmentMode&&<button className="icon-btn danger-button" title="ลบข้อมูลทดลอง" onClick={()=>forceDeleteProduct(p)}><Trash2 size={16}/></button>}</div></td>
        </tr>})}</tbody>
      </table>{!filteredProducts.length&&<Empty text="ไม่พบสินค้าตามสถานะที่เลือก"/>}</div>
    </section></div>}
    ...
    {page==='prepack'&&<div className="content prepack-page">
      {!prepackReady&&<section className="info-box prepack-warning"><strong>ยังไม่ได้เปิดใช้งานฐานข้อมูลพรีแพ็ค</strong><span>กรุณารันไฟล์ supabase/migration-v3.1-prepack.sql ใน Supabase SQL Editor แล้วกดรีเฟรช</span></section>}
      <section className="metrics prepack-metrics">
        <article><PackageCheck/><span>สินค้าในพรีแพ็ค</span><strong>{prepackSummary.skuCount.toLocaleString()}</strong></article>
        <article className="success-card"><Boxes/><span>จำนวนพรีแพ็ครวม</span><strong>{prepackSummary.total.toLocaleString()}</strong></article>
        <article><ArrowUpFromLine/><span>พร้อมส่งแล้ววันนี้</span><strong>{prepackSummary.shippedToday.toLocaleString()}</strong></article>
      </section>
      <section className="panel">
        <div className="panel-title prepack-panel-title">
          <div><h2>คลังพรีแพ็ค</h2><p>สินค้าที่เบิกจากคลังกลางมาแพ็ครอติดลาเบลพร้อมส่งลูกค้า</p></div>
          <div className="toolbar prepack-toolbar">
            <div className="search"><Search size={17}/><input value={prepackQuery} onChange={e=>setPrepackQuery(e.target.value)} placeholder="ค้นหาสินค้าพรีแพ็ค"/></div>
            <button className="btn primary" disabled={!prepackReady} onClick={()=>openPrepackModal('pack')}><PackagePlus size={18}/>นำเข้าพรีแพ็ค</button>
          </div>
        </div>
        <div className="table-wrap"><table className="prepack-table">
          <thead><tr><th>สินค้า</th><th>หมวดหมู่</th><th>คลังกลาง</th><th>คงเหลือพรีแพ็ค</th><th>อัปเดตล่าสุด</th><th>จัดการ</th></tr></thead>
          <tbody>{filteredPrepack.map(row=><tr key={row.product_id}>
            <td><div className="product-name-cell">{row.products?.image_path?<img className="product-thumb" src={productImageUrl(row.products.image_path)} alt={row.products?.name}/>:<span className="product-thumb placeholder"><ImageIcon size={18}/></span>}<strong>{row.products?.name||'-'}</strong></div></td>
            <td>{row.products?.categories?.name||'-'}</td>
            <td>{Number(row.products?.quantity||0).toLocaleString()} {row.products?.unit||''}</td>
            <td><strong className="prepack-quantity">{Number(row.quantity||0).toLocaleString()}</strong> {row.products?.unit||''}</td>
            <td>{new Date(row.updated_at).toLocaleString('th-TH')}</td>
            <td><div className="row-actions">
              <button className="btn mini success-action" onClick={()=>openPrepackModal('ship',{...row.products,prepack_quantity:row.quantity})}><PackageCheck size={15}/>พร้อมส่งแล้ว</button>
              <button className="btn mini warning-action" onClick={()=>openPrepackModal('return',{...row.products,prepack_quantity:row.quantity})}><RotateCcw size={15}/>คืนคลัง</button>
            </div></td>
          </tr>)}</tbody>
        </table>{!filteredPrepack.length&&<Empty text={prepackReady?'ยังไม่มีสินค้าในพรีแพ็ค':'รอเปิดใช้งานฐานข้อมูลพรีแพ็ค'}/>}</div>
      </section>
      <section className="panel">
        <div className="panel-title"><div><h2>ประวัติพรีแพ็ค</h2><p>รายการย้ายเข้า พร้อมส่ง และคืนกลับคลังกลางล่าสุด</p></div></div>
        <div className="table-wrap"><table className="prepack-history-table">
          <thead><tr><th>วันเวลา</th><th>เอกสาร</th><th>สินค้า</th><th>รายการ</th><th>จำนวน</th><th>คงเหลือพรีแพ็ค</th><th>คงเหลือคลังกลาง</th><th>ผู้ทำรายการ</th><th>หมายเหตุ</th></tr></thead>
          <tbody>{prepackTransactions.map(t=><tr key={t.id}>
            <td>{new Date(t.created_at).toLocaleString('th-TH')}</td><td><code>{t.document_no}</code></td><td><strong>{t.products?.name||'-'}</strong></td>
            <td><span className={`badge ${prepackBadge(t.transaction_type)}`}>{PREPACK_LABELS[t.transaction_type]||t.transaction_type}</span></td>
            <td>{Number(t.quantity).toLocaleString()} {t.products?.unit||''}</td><td>{Number(t.prepack_after).toLocaleString()}</td><td>{Number(t.stock_after).toLocaleString()}</td><td>{t.actor_name||'-'}</td><td>{t.note||'-'}</td>
          </tr>)}</tbody>
        </table>{!prepackTransactions.length&&<Empty text="ยังไม่มีประวัติพรีแพ็ค"/>}</div>
      </section>
    </div>}
    <Footer/></main>

    <Modal open={productModal} title={editing?'แก้ไขสินค้า':'เพิ่มสินค้า'} onClose={()=>{setProductModal(false);setEditing(null);setImagePreview('')}}>
      <form className="modal-form" onSubmit={saveProduct}>
        <div className="image-upload"><div className="image-preview">{imagePreview?<img src={imagePreview} alt="ตัวอย่างรูปสินค้า"/>:<ImageIcon size={34}/>}</div><label>รูปสินค้า<input name="image" type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>{const file=e.target.files?.[0];if(file)setImagePreview(URL.createObjectURL(file))}}/><small>JPG, PNG หรือ WebP ขนาดไม่เกิน 2 MB</small></label></div>
        <label>ชื่อสินค้า<input name="name" defaultValue={editing?.name||''} required/></label>
        <label>จำนวนชิ้นต่อลัง<input name="units_per_case" type="number" min="1" value={piecesPerCase} onChange={e=>setPiecesPerCase(Math.max(1,Number(e.target.value)||1))} required/><small>กำหนดว่าสินค้านี้ 1 ลังบรรจุกี่ชิ้น</small></label>
        {!editing?<><div className="form-grid"><label>จำนวนลังเริ่มต้น<input name="case_count" type="number" min="0" value={caseCount} onChange={e=>setCaseCount(Math.max(0,Number(e.target.value)||0))}/></label><label>เศษชิ้นเริ่มต้น<input name="loose_quantity" type="number" min="0" value={loosePieces} onChange={e=>setLoosePieces(Math.max(0,Number(e.target.value)||0))}/></label></div><p className="case-conversion-preview"><span>คงเหลือเริ่มต้น</span><strong>{(caseCount*piecesPerCase+loosePieces).toLocaleString()} ชิ้น</strong><small>{caseCount.toLocaleString()} ลัง × {piecesPerCase.toLocaleString()} ชิ้น {loosePieces>0?`+ เศษ ${loosePieces.toLocaleString()} ชิ้น`:''}</small></p></>:<p className="info-box">คงเหลือปัจจุบัน {Number(editing.quantity||0).toLocaleString()} ชิ้น — การแก้จำนวนชิ้นต่อลังจะไม่เปลี่ยนยอดสต็อกปัจจุบัน</p>}
        <label>สถานะสินค้า<select name="is_active" defaultValue={editing?.is_active===false?'false':'true'}><option value="true">ใช้งานปกติ</option><option value="false">ไม่ใช้งานแล้ว</option></select><small>สินค้าที่ไม่ใช้งานแล้วจะยังเก็บประวัติไว้ แต่จะไม่สามารถเบิก เติม หรือตีกลับสินค้าได้</small></label>
        <label>แจ้งเตือนสินค้าใกล้หมดเมื่อเหลือไม่เกิน<input name="min_stock" type="number" min="0" defaultValue={editing?.min_stock||0}/><small>ระบุเป็นจำนวนชิ้น และใส่ 0 เพื่อปิดการแจ้งเตือน</small></label>
        <div className="modal-actions"><button type="button" className="btn secondary" onClick={()=>{setProductModal(false);setEditing(null)}}>ยกเลิก</button><button className="btn primary" disabled={busy}>บันทึก</button></div>
      </form>
    </Modal>

    <Modal open={txModal} title="ทำรายการสต็อก" onClose={()=>setTxModal(false)}>
      <form className="modal-form" onSubmit={saveTx}>
        <div className="product-summary"><PackagePlus/><div><strong>{txProduct?.name}</strong><small>คงเหลือ {Number(txProduct?.quantity||0).toLocaleString()} ชิ้น · 1 ลัง = {Number(txProduct?.units_per_case||1).toLocaleString()} ชิ้น</small></div></div>
        <label>ประเภทรายการ<select name="type"><option value="issue">เบิกสินค้า</option><option value="restock">เติมสินค้า</option><option value="returned">สินค้าตีกลับ</option></select></label>
        <div className="form-grid"><label>นับจำนวนเป็น<select name="quantity_mode" defaultValue={Number(txProduct?.units_per_case||1)>1?'case':'piece'}><option value="case">ลัง</option><option value="piece">ชิ้น</option></select></label><label>จำนวน<input name="quantity" type="number" min="1" required autoFocus/></label></div>
        <p className="info-box">ถ้าเลือก “ลัง” ระบบจะนำจำนวนลังคูณ {Number(txProduct?.units_per_case||1).toLocaleString()} แล้วบันทึกยอดสต็อกเป็นชิ้น</p>
        <label>หมายเหตุ<textarea name="note" placeholder="ระบุรายละเอียดเพิ่มเติม (ไม่บังคับ)"/></label>
        <div className="modal-actions"><button type="button" className="btn secondary" onClick={()=>setTxModal(false)}>ยกเลิก</button><button className="btn primary" disabled={busy}>ยืนยันรายการ</button></div>
      </form>
    </Modal>
    ...
    <Modal open={prepackModal} title={prepackAction==='pack'?'นำสินค้าเข้าพรีแพ็ค':prepackAction==='ship'?'ยืนยันสินค้าพร้อมส่ง':'คืนสินค้าเข้าคลังกลาง'} onClose={()=>{setPrepackModal(false);setPrepackProduct(null)}}>
      <form className="modal-form" onSubmit={savePrepack}>
        {prepackAction==='pack'?<>
          <p className="info-box">รายการนี้จะหักสินค้าออกจากคลังกลางและเพิ่มเข้าคลังพรีแพ็คทันที</p>
          <label>สินค้า<select name="product_id" required autoFocus><option value="">เลือกสินค้า</option>{products.filter(p=>p.is_active&&Number(p.quantity||0)>0).map(p=><option key={p.id} value={p.id}>{p.name} — คลังกลาง {Number(p.quantity).toLocaleString()} {p.unit}</option>)}</select></label>
        </>:<div className="product-summary"><PackageCheck/><div><strong>{prepackProduct?.name}</strong><small>คงเหลือพรีแพ็ค {Number(prepackProduct?.prepack_quantity||0).toLocaleString()} {prepackProduct?.unit}</small></div></div>}
        <div className="form-grid"><label>นับจำนวนเป็น<select name="quantity_mode" defaultValue="piece"><option value="piece">ชิ้น</option><option value="case">ลัง</option></select></label><label>จำนวน<input name="quantity" type="number" min="1" max={prepackAction==='pack'?undefined:Number(prepackProduct?.prepack_quantity||0)} required autoFocus={prepackAction!=='pack'}/></label></div>
        <label>หมายเหตุ<textarea name="note" placeholder={prepackAction==='pack'?'เช่น แพ็คสำรองสำหรับออเดอร์ออนไลน์':prepackAction==='ship'?'เช่น ติดลาเบลและส่งมอบให้ขนส่งแล้ว':'เช่น ยกเลิกพรีแพ็คหรือแพ็คผิดรุ่น'}/></label>
        <div className="modal-actions"><button type="button" className="btn secondary" onClick={()=>{setPrepackModal(false);setPrepackProduct(null)}}>ยกเลิก</button><button className="btn primary" disabled={busy}>{busy?<Spinner/>:prepackAction==='pack'?'ย้ายเข้าพรีแพ็ค':prepackAction==='ship'?'ยืนยันพร้อมส่ง':'คืนเข้าคลัง'}</button></div>
      </form>
    </Modal>
