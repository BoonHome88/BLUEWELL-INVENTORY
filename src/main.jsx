...
const forceDeleteProduct = async product => {
  ...
  const reportSummary=useMemo(()=>{
    ...
    return {restock,issue,net:restock-issue,count:reportRows.length}
  },[reportRows])
  ...
  const dashboardData=useMemo(()=>{
    const days=[]
    for(let i=6;i>=0;i--){const d=new Date();d.setHours(0,0,0,0);d.setDate(d.getDate()-i);days.push({date:d,label:d.toLocaleDateString('th-TH',{day:'2-digit',month:'short'}),restock:0,issue:0})}
    transactions.forEach(t=>{const d=new Date(t.created_at);const row=days.find(x=>d>=x.date&&d<new Date(x.date.getTime()+86400000));if(row)row[t.transaction_type]+=Number(t.quantity||0)})
    return days
  },[transactions])
  ...
  const reportData=()=>reportRows.map(t=>({
    ...
    'ประเภท':t.transaction_type==='issue'?'เบิก':'เติม',
  ...
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><div className="brand-mark small">B</div><div><strong>{company?.company_name||'BlueWell Inventory'}</strong><small>{company?.subtitle||'ระบบจัดการสต็อกออนไลน์'}</small></div></div><nav>{nav.map(([id,label,Icon])=><button key={id} className={page===id?'active':''} onClick={()=>setPage(id)}><Icon size={19}/>{label}</button>)}</nav><div className="user-card"><CircleUserRound/><div><strong>{displayName}</strong><small>{isAdmin?'ผู้ดูแลระบบ':'พนักงาน'}</small></div><button className="icon-btn" onClick={logout}><LogOut size={18}/></button></div></aside>
    ...
    {page==='history'&&<div className="content"><section className="panel"><div className="panel-title"><div><h2>ประวัติการเคลื่อนไหว</h2><p>รายการเบิกและเติมสต็อกล่าสุด</p></div><div className="toolbar"><select value={typeFilter} onChange={e=>setTypeFilter(e.target.value)}><option value="">ทั้งหมด</option><option value="issue">เบิก</option><option value="restock">เติม</option></select><div className="search"><Search size={17}/><input value={historyQuery} onChange={e=>setHistoryQuery(e.target.value)} placeholder="ค้นหาเอกสารหรือสินค้า"/></div></div></div><div className="table-wrap"><table><thead><tr><th>วันเวลา</th><th>เอกสาร</th><th>สินค้า</th><th>ประเภท</th><th>จำนวน</th><th>คงเหลือ</th><th>ผู้ทำรายการ</th><th>เคลม</th><th>หมายเหตุ</th></tr></thead><tbody>{filteredTx.map(t=><tr key={t.id}><td>{new Date(t.created_at).toLocaleString('th-TH')}</td><td><code>{t.document_no}</code></td><td><strong>{t.products?.name||'-'}</strong></td><td><span className={`badge ${t.transaction_type==='issue'?'danger':'success'}`}>{t.transaction_type==='issue'?'เบิก':'เติม'}</span></td><td>{t.quantity} {t.products?.unit||''}</td><td>{t.balance_after}</td><td>{t.actor_name}</td><td>{t.transaction_type==='issue'?<div className="claim-link"><span>{Number(claimedByIssue[t.id]||0).toLocaleString()} / {Number(t.quantity).toLocaleString()}</span>{isAdmin&&Number(claimedByIssue[t.id]||0)<Number(t.quantity)&&<button className="btn mini" onClick={()=>{setClaimIssue(t);setClaimModal(true)}}><ShieldAlert size={14}/>แจ้งชำรุด</button>}</div>:'-'}</td><td>{t.note||'-'}</td></tr>)}</tbody></table>{!filteredTx.length&&<Empty/>}</div></section></div>}
    ...
    <Modal open={txModal} title="เบิก / เติมสินค้า" onClose={()=>setTxModal(false)}><form className="modal-form" onSubmit={saveTx}><div className="product-summary"><PackagePlus/><div><strong>{txProduct?.name}</strong><small>คงเหลือ {txProduct?.quantity} {txProduct?.unit}</small></div></div><label>ประเภทรายการ<select name="type"><option value="issue">เบิกสินค้า</option><option value="restock">เติมสินค้า</option></select></label><label>จำนวน<input name="quantity" type="number" min="1" required autoFocus/></label><label>หมายเหตุ<textarea name="note" placeholder="ระบุวัตถุประสงค์หรือผู้รับสินค้า"/></label><div className="modal-actions"><button type="button" className="btn secondary" onClick={()=>setTxModal(false)}>ยกเลิก</button><button className="btn primary" disabled={busy}>ยืนยันรายการ</button></div></form></Modal>
