const key = name => `bluewell-offline-${name}`;
let products = JSON.parse(localStorage.getItem(key("products")) || "[]");
let parcels = JSON.parse(localStorage.getItem(key("parcels")) || "[]");
...
function renderUpdated(){
  const tp=parcels.filter(x=>dateKey(x.created_at)===today());
  ...
  $("historyRows").innerHTML=history.map(x=>`<tr><td><span class="badge">${x.type}</span></td><td>${safe(x.name)}</td><td>${safe(x.quantity)}</td><td>${safe(x.note)}</td><td>${safe(x.operator)}</td><td>${new Date(x.time).toLocaleString("th-TH")}</td></tr>`).join("")||`<tr><td colspan="6">ยังไม่มีประวัติ</td></tr>`;
}
$("issueRows").addEventListener("click",e=>{
  const button=e.target.closest("[data-delete-issue]");
  if(!button)return;
